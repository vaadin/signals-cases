...
@PageTitle("UC3 — Copy image")
@Menu(order = 3, title = "UC3 — Copy image")
public class CopyImageView extends VerticalLayout {
    ...
    public CopyImageView() {
        add(new H1("UC3 — Copy image"));
        add(new Paragraph(
                "The image is fetched client-side from its source element and "
                        + "written to the clipboard inside the click handler, so "
                        + "the user gesture is preserved."));
