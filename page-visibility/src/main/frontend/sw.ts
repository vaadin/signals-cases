...
async function focusOrOpenWindow() {
  // Where the notification should land if there is no open tab.
  const fallbackUrl = new URL('/uc3', self.location.origin).href;

  // Look for any window already open on this origin (any path, any scope).
  ...
  const appWindow = allWindows.find(
