...
/**
 * UC29 — Konami code Easter egg via SequenceTrigger.
 * <p>
 * A {@link SequenceTrigger} tracks the ordered sequence
 * ↑ ↑ ↓ ↓ ← → ← → B A client-side. Only when the user completes the full
 * sequence does the trigger fire a {@link CallbackAction} that bumps a
 * counter and reveals an Easter-egg message. Partial progress never
 * crosses the network — the state lives in a closure inside the install
 * JS.
 * <p>
 * Demonstrates {@code SequenceTrigger} (a local port of the
 * feature/triggers-actions class).
 */
@Route(value = "uc29", layout = MainLayout.class)
@PageTitle("UC29 — Konami code")
@Menu(order = 29, title = "UC29 — Konami code")
@StyleSheet("uc29.css")
public class KonamiCodeView extends VerticalLayout {
    ...
    public KonamiCodeView() {
        addClassName("uc29-view");
        add(new H1("UC29 — Konami code"));
        add(new Paragraph(
                "Type the Konami code anywhere on this page: "
        ...
        new SequenceTrigger(this, Key.ARROW_UP, Key.ARROW_UP, Key.ARROW_DOWN,
                ...
                Key.ARROW_LEFT, Key.ARROW_RIGHT, Key.KEY_B, Key.KEY_A)
